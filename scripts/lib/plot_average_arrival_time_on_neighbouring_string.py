...
dom_numbers = range(1, 60)
weighted_mean_arrival_times = []
for dom_number in dom_numbers:
  d = flasher_data[flasher_data.dom_number == dom_number]
  weighted_mean_arrival_times.append((d["time"] * d["charge"]).mean())
...
for i, simulation_title in enumerate(simulation_titles):
  ...
  ax.plot(
    ...
    list(simulation_data.groupby(["dom"])["time"].mean()),
