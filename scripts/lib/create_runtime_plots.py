...
for r in [0.1, 0.5, 1.0, 2.0]:
  ...
  y = subplot_data["duration"] / 60 # minutes

  # line plot
  ax0.plot(x, y, label = "hole-ice radius = " + str(r) + " dom radii")

  # labels
  ax0.set(xlabel = "effective scattering length [m]", ylabel="Job execution time [min]",
...
for s in [0.020000000000000004, 0.35, 1.0, 3.0]:
  ...
  # line plot
  ax1.plot(x, y, label = "effective scattering length = " + str(round(s, 2)) + "m")

  # labels
  ax1.set(xlabel = "hole-ice radius [dom radii]", ylabel="Job execution time [min]")
